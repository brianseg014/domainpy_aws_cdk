...
from domainpy_aws_cdk.edge.base import IGateway, IChannelHook, IIntegrationEventSubscription
...
class SnsTopicIntegrationEventSubscription(IIntegrationEventSubscription):
    ...
    def bind(self, gateway: IGateway):
        if isinstance(gateway, RestApiGateway):
            ...
            raise Exception('gateway-channel incompatible')

    def _bind_rest_api_gateway(self, gateway: RestApiGateway):
        integration_event_topic = self.channel.topic
        resolver_queue = gateway.resolver_queue
        resolver_dlq = gateway.resolver_dlq
